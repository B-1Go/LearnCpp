...
int main()
{
	...
	int i = 100;
	int* pInt = nullptr; // nullptr 은 사실 0이다.
	float f = 3.f;

	//pInt = &i;
	pInt = (int*)&f; // 문법적으로 IDE가 막아주지만 (int*)캐스팅으로 강제로 넣어준다.


	// 포인터는 딱 주소 변수만 저장하는데
	// 그 주소에 몇 바이트나 접근할 건지 가지고 있지는 않다.
	// 그래서 주소변수 앞에 int*를 적으므로써
	// 내가 접근한 주소에 데이터는 4바이트이고 정수형 이라는 것 까지 의미를 담고있다.
	// 따라서 pInt에 다른 자료형으로 접근을 하게되면 문제가 생긴다.
